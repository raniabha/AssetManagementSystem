...
class UserPendingAsset extends Component {
  constructor(props) {
    super(props);
    this.state = {
      error: null,
  ...
  componentDidMount() {
    axios
      ...
      .then(result => {
        this.setState({
                assets: result.data.assets.filter(asset => asset.status === "pending"),
              })
      })
      .catch(error => {
        this.setState({ error });
  ...
  render() {
    ...
    if(error) {
      ...
      return(
        ...
        <Container>
        <div>
          <h2 style={{textAlign:"center"}}>Pending Asset </h2>
...
export default UserPendingAsset;
